...
<%
   String remoteImageUrl = "http://"+request.getServerName()+":"+request.getServerPort()+"/mytest/static/img/logo.jpg";
...
<body>
		...
		<p>
			    1. 静态内容Servlet, 演示高效读取静态内容, 控制客户端缓存, 压缩传输, 弹出下载对话框, 见StaticContentServlet.<br/>
			       <img src="${ctx}/static-content?contentPath=static/img/logo.jpg"/> <a href="${ctx}/static-content?contentPath=static/img/logo.jpg&download=true">图片下载链接</a><br/>
			    ...
			    #########################################################<br/>
			    以上全部没有看到有什么优化现象<br><br><br>
			    4. YUI Compressor 压缩js/css,见bin/yuicompressor.bat命令及webapp中两个版本的js/css文件<br/>
		</p>
		<hr/>
		<h3>Ajax演示</h3>
		<p>
			<a href="${ctx}/sc/web/mashup?callback">跨域名Mashup演示</a> 演示基于JSONP Mashup 跨域名网站的内容.
			<br>
			#########################################################<br/>
			感觉像普通的ajax操作，没有什么新东西，因为sitemesh原因，返回结果包含HTML模板内容
		</p>
</body>
</html>