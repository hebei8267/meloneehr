...
import cn.com.free.framework.generator.util.FileHelper;
import cn.com.free.framework.generator.util.FileUtils;
import cn.com.free.framework.generator.util.IOHelper;
...
public class Generator {
		...
		for (int i = 0; i < templateFiles.size(); i++) {
			...
			if ((testExpressionIndex = templateRelativePath.indexOf('@')) != -1) {
				...
				if (expressionValue == null) {
					System.err.println("[not-generate] WARN: test expression is null by key:[" + testExpressionKey + "] on template:["
	...
	private void generateNewFileOrInsertIntoFile(final IGeneratorModelProvider modelProvider, final Configuration config,
			final String templateFile, final String outputFilePath) throws Exception {
	...
	private boolean isFoundInsertLocation(final Template template, final Map model, final File outputFile, final StringWriter newFileContent)
	...
	private void saveNewOutputFileContent(final Template template, final Map model, final File outputFile) throws IOException,
			TemplateException {
