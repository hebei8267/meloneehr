...
public abstract class HtmlBasicRenderer extends Renderer {
		...
		if (!(component instanceof UIInput)) {
			...
			if (logger.isLoggable(Level.FINE)) {
				logger.log(Level.FINE,
						"No decoding necessary since the component {0} is not an instance or a sub class of UIInput", component
								.getId());
			}
			return;
		}
	 ...
	 * </p>
	 * <p>
	 * If the <code>forValue</code> doesn't already include a generated
	 * suffix, but the id of the <code>fromComponent</code> does include a
	 * generated suffix, then append the suffix from the
	 * <code>fromComponent</code> to the <code>forValue</code>. Otherwise
	...
	/**
	 * Renderers override this method to write appropriate HTML content into the
	 * buffer.
	 * 
	 * @param context the FacesContext for the current request
	...
	protected UIComponent getForComponent(FacesContext context, String forComponent, UIComponent component) {
		...
		if (result == null) {
			if (logger.isLoggable(Level.WARNING)) {
				logger.warning(MessageUtils.getExceptionMessageString(MessageUtils.COMPONENT_NOT_FOUND_IN_VIEW_WARNING_ID,
						forComponent));
			}
		}
		return result;
	 ...
	 * 
	 * @param component the target component to which the submitted value will
	 *            be set
	...
	protected boolean shouldDecode(UIComponent component) {
		...
		if (Util.componentIsDisabledOrReadonly(component)) {
			if (logger.isLoggable(Level.FINE)) {
				logger.log(Level.FINE, "No decoding necessary since the component {0} is disabled or read-only", component
						.getId());
			}
			return false;
		}
