...
public class RoleDao extends HibernateDaoImpl<Role> {
	...
	public Role getRoleByID(String roleID) {
		List<Role> resultList = getHibernateTemplate().findByNamedQuery(
	...
	public long getRole4UserCount(List<String> list) {
		...
		//getHibernateTemplate().findByNamedQuery("Role.getRole4UserCount");
		// if (resultList.size() > 0) {
	...
	public List<Role> getRoleListByMenuNodeID(String menuNodeID) {
		return getHibernateTemplate().findByNamedQuery(
				"Role.getRoleListByMenuNodeID", menuNodeID);
	}
}
