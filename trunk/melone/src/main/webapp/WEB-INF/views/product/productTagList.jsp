		...
		<script>
			$().ready(function() {
				...
				$("#newBtn").click(function() {
					$(location).attr('href', '${sc_ctx}/product/productTag/new');
				});
						...
						$("#__del-confirm").dialog({
							...
							buttons: {
								"删除": function() {
									...
									$("#uuids").val(uuids);
									$("#listForm").attr("action", "${sc_ctx}/product/productTag/del");
						        	$("#listForm").submit();
								},
								"关闭": function() {
									$( this ).dialog( "close" );
	...
	<body>
		...
		<div class="grid_16">
			...
			<table id="rounded-corner">
				...
				<tbody>
					<c:if test="${empty productTagList}" >
					<tfoot>
						<tr>
					...
					<c:forEach items="${productTagList}" var="productTag">
					<tr>
						<td class="first">
							<input type="checkbox" name="uuid" value="${productTag.uuid}"></input>
						</td>
						<td>
							${productTag.name}&nbsp;
						</td>
						...
						<td align="center">
							<a href="${sc_ctx}/product/productTag/edit/${productTag.uuid}">编辑</a>
						</td>
					</tr>
					</c:forEach>
				</tbody>
