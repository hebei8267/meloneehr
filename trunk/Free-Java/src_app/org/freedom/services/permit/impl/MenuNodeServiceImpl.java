...
import org.freedom.sys.modules.MenuTreeCheckBoxNode;
import org.freedom.sys.modules.MenuTreeNode;
...
public class MenuNodeServiceImpl implements IMenuNodeService {
            ...
            if (parentMenuNode != null) {
                ...
                if (_check) {
                    ...
                    if (inheritFlg && Boolean.FALSE.equals(menuNode.getDefaultPermit())) {
                        List<MenuNodePermit> permitList = menuNodePermitDao.getMenuNodePermitListByMenuNodeID(parentMenuNode.getId());
    ...
    private boolean menuNodeTypeCheck(MenuNode parentMenuNode, MenuNode childMenuNode) {
        ...
        if (MenuNodeType.AREA_NODE_TYPE.equals(parentMenuNode.getNodeType())
            ...
            if (!(MenuNodeType.FOLDER_NODE_TYPE.equals(childMenuNode.getNodeType()) || MenuNodeType.LEAF_NODE_TYPE.equals(childMenuNode
                    .getNodeType()))) {
                return false;
            }
        }
    ...
    public TreeNode getMenuNodeInfoTreeEmbodyPermitService(String rootID, String roleID) {
        ...
        MenuNode dbNodeRoot = menuNodeDao.getMenuNodeByID(rootID);
        if (dbNodeRoot != null) {
            MenuTreeCheckBoxNode menuTreeNode = new MenuTreeCheckBoxNode();
     ...
     * @param roleID 角色ID
     */
    private void buildMenuNodeInfoTree(MenuTreeCheckBoxNode parentNode, MenuNode dbParentNode, List<MenuNodePermit> permitList,
            String roleID) {
     ...
     * @param dbParentNode 数据库中取得的菜单节点
     */
    private void buildMenuNodeInfoTree(MenuTreeNode parentNode, MenuNode dbParentNode) {
        for (MenuNode dbNode : dbParentNode.getChildNodeList()) {
            if (dbNode != null) {
                MenuTreeNode menuTreeNode = new MenuTreeNode();
     ...
     * @param roleID 角色ID
     */
    private void buildMenuNodeInfoTree(MenuTreeNode parentNode, MenuNode dbParentNode, List<String> roleMenuNodePermitList, String roleID) {
        for (MenuNode dbNode : dbParentNode.getChildNodeList()) {
