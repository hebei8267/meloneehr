...
/**
 * <p>
 * <strong>HtmlResponseWriter</strong> is an Html specific implementation of
 * the <code>ResponseWriter</code> abstract class. Kudos to Adam Winer
 * (Oracle) for much of this code.
 */
...
public class HtmlResponseWriter extends ResponseWriter {
	 ...
	 * 
	 * <p>
	 * BooleanWebContextInitParameter.EnableJSStyleHiding: <code>true</code>
	 ...
	 * @throws javax.faces.FacesException the encoding is not recognized.
	 */
	public HtmlResponseWriter(Writer writer, String contentType, String encoding, Boolean isScriptHidingEnabled,
		...
		if (isScriptInAttributeValueEnabled == null) {
			webConfig = getWebConfiguration(webConfig);
			isScriptInAttributeValueEnabled = (null == webConfig) ? BooleanWebContextInitParameter.EnableScriptInAttributeValue
		...
		if (disableUnicodeEscaping == null) {
			...
			disableUnicodeEscaping = WebConfiguration.DisableUnicodeEscaping
					.getByValue((null == webConfig) ? WebConfiguration.WebContextInitParameter.DisableUnicodeEscaping
							.getDefaultValue() : webConfig
							.getOptionValue(WebConfiguration.WebContextInitParameter.DisableUnicodeEscaping));
			if (disableUnicodeEscaping == null) {
	...
	public void endElement(String name) throws IOException {
		...
		if (isScriptOrStyle(name) && !scriptOrStyleSrc && writer instanceof FastStringWriter) {
			...
			if (result != null) {
				...
				if (isXhtml) {
					if (isScript) {
						Matcher cdataStartSlashSlash = CDATA_START_SLASH_SLASH.matcher(trim), cdataEndSlashSlash = CDATA_END_SLASH_SLASH
								.matcher(trim), cdataStartSlashStar = CDATA_START_SLASH_STAR.matcher(trim), cdataEndSlashStar = CDATA_END_SLASH_STAR
								.matcher(trim);
	 ...
	 * been created by a call to <code>startElement()</code>, that element
	 ...
	 * <code>writeText(c, 0, c.length)</code>. If there is an open element
	 * that has been created by a call to <code>startElement()</code>, that
	 * element will be closed first.
	 * </p>
	 * </p>
	...
	public void writeURIAttribute(String name, Object value, String componentPropertyName) throws IOException {
		...
		if (stringValue.startsWith("javascript:")) {
			...
			HtmlUtils.writeURL(attributesBuffer, stringValue, textBuffer, encoding, getContentType());
		}
