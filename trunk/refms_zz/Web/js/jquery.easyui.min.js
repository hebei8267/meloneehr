...
$.fn.datagrid.defaults={title:null,iconCls:null,border:true,width:"auto",height:"auto",frozenColumns:null,columns:null,striped:false,method:"post",nowrap:true,idField:null,url:null,loadMsg:"正在处理，请稍待。。。",pagination:false,rownumbers:false,singleSelect:false,fit:false,pageNumber:1,pageSize:10,pageList:[10,20,30,40,50],queryParams:{},sortName:null,sortOrder:"asc",onLoadSuccess:function(){
},onLoadError:function(){
...
},beforePageText:"第",afterPageText:"页/共{pages}页",displayMsg:"显示{from}到{to},共{total}记录"};
})(jQuery);
(function($){
function _1a9(node){
node.each(function(){
...
if(item.attributes!=null && item.attributes.ckName!=null){
	if(item.attributes.ckChecked&&item.attributes.ckChecked==true){
		$('<span class=\"tree-title\"></span>').html("<input type=\"checkbox\" name="+item.attributes.ckName+" value="+item.id+" checked=\"true\">"+item.text).appendTo(node);
	}else{
		$('<span class=\"tree-title\"></span>').html("<input type=\"checkbox\" name="+item.attributes.ckName+" value="+item.id+" >"+item.text).appendTo(node);
	}
}else{
