...
import org.springside.examples.miniservice.webservice.dto.UserDTOList;
...
public class UserResouceService {
	...
	public UserDTO getUser(@PathParam("id") Long id) {
		try {
			User entity = miniAccountManager.getUser(id);
	...
	@Produces({ MediaType.APPLICATION_JSON,
			MediaType.APPLICATION_XML + WsConstants.CHARSET })
	public UserDTOList searchUser(@QueryParam("loginName") String loginName,
			@QueryParam("name") String name) {
		try {
			List<User> entityList = miniAccountManager.searchUser(name);

			UserDTOList userDtoList = new UserDTOList();
			userDtoList.setUsers(entityList);
			return userDtoList;
		} catch (RuntimeException e) {
			throw RsResponse.buildDefaultException(e);
		}
	...
	public Response createUser(UserDTO user) {
		...
		try {
			...
			Long id = miniAccountManager.saveUser(userEntity);
			...
			String message = StringUtils.join(
					BeanValidators.extractPropertyAndMessage(e), "\n");
